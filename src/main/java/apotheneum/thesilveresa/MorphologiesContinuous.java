...
@LXComponent.Name("Morphologies-Continuous")
public class MorphologiesContinuous extends ApotheneumPattern {
  ...
  public MorphologiesContinuous(LX lx) {
    super(lx);
  ...
  protected void render(double deltaMs) {
    ...
    // Continuous, unwrapped elapsed time (double precision). The previous
    // implementation used (nowMillis % 30000) / 1000f as the time base for the
    // flow sin/cos offsets. Since that wraps every 30s but the sin/cos periods
    // depend on the user-adjustable Flow parameter, the wrap point almost never
    // lands on a whole cycle - every LED's flow offset would snap to a different
    // value simultaneously every 30 seconds, producing a small, periodic,
