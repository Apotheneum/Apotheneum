package apotheneum.thesilveresa;

import apotheneum.Apotheneum;
import apotheneum.Apotheneum.Cube;
import apotheneum.Apotheneum.Cube.Face;
import apotheneum.Apotheneum.Cube.Row;
import apotheneum.Apotheneum.Cylinder;
import apotheneum.Apotheneum.Cylinder.Ring;
import apotheneum.ApotheneumPattern;
import heronarts.lx.LX;
import heronarts.lx.LXCategory;
import heronarts.lx.LXComponent;
import heronarts.lx.color.LXColor;
import heronarts.lx.parameter.BooleanParameter;
import heronarts.lx.parameter.CompoundParameter;
import heronarts.lx.parameter.TriggerParameter;

import java.util.Random;

@LXCategory("Apotheneum/thesilveresa")
@LXComponent.Name("Planetary Ring")
public class PlanetaryRing extends ApotheneumPattern {

  private static final int FW = 128, FH = 128;
  private static final int MAX_P = 4000;
  private static final float TWO_PI = 6.2831855f;

  // Orbits

  private final TriggerParameter reseed = new TriggerParameter("Reset", this::resetBodies)
    .setDescription("Put every body back where it started");

  private final CompoundParameter bodies = new CompoundParameter("Bodies", 60, 5, 100)
    .setDescription("How much matter is in orbit");
  private final CompoundParameter inner = new CompoundParameter("Inner", 12, 0, 60)
    .setDescription("Inner edge of the disk");
  private final CompoundParameter outer = new CompoundParameter("Outer", 42, 5, 90)
    .setDescription("Outer edge of the disk");
  private final CompoundParameter gaps = new CompoundParameter("Gaps", 0, 0, 100)
    .setDescription("Resonance gaps carved in the ring");
  private final CompoundParameter gapCount = new CompoundParameter("GapN", 3, 1, 8)
    .setDescription("How many gaps");
  private final CompoundParameter gapSharp = new CompoundParameter("GapEdge", 55, 0, 100)
    .setDescription("How crisply the gaps cut - high gives a true Cassini division");
  private final CompoundParameter striate = new CompoundParameter("Striate", 0, 0, 100)
    .setDescription("Fine concentric ringlets within the bands");
  private final CompoundParameter striateN = new CompoundParameter("StriN", 22, 3, 90)
    .setDescription("How many ringlets");
  private final CompoundParameter thickness = new CompoundParameter("Thick", 25, 1, 100)
    .setDescription("Vertical thickness of the disk");
  private final CompoundParameter stretchX = new CompoundParameter("StretchX", 100, 25, 400)
    .setDescription("Widen the rings outward, inner edge held in place");
  private final CompoundParameter stretchY = new CompoundParameter("StretchY", 100, 25, 400)
    .setDescription("Stretch the rings vertically, inner edge held in place");
  private final CompoundParameter tilt = new CompoundParameter("Tilt", 55, 0, 100)
    .setDescription("Viewing angle - 0 is edge on, 100 is face on");

  // Motion

  private final CompoundParameter rate = new CompoundParameter("Rate", 40, 0, 100)
    .setDescription("Orbital speed");
  private final CompoundParameter kepler = new CompoundParameter("Kepler", 75, 0, 100)
    .setDescription("How much faster the inner orbits run - real gravity is 100");
  private final CompoundParameter eccent = new CompoundParameter("Ecc", 0, 0, 90)
    .setDescription("Orbital eccentricity");
  private final CompoundParameter precess = new CompoundParameter("Prec", 15, 0, 100)
    .setDescription("How fast the ellipses rotate");

  // Accretion

  private final CompoundParameter infall = new CompoundParameter("Infall", 0, 0, 100)
    .setDescription("Matter spiralling inward onto the centre");
  private final CompoundParameter heat = new CompoundParameter("Heat", 45, 0, 100)
    .setDescription("How much the inner disk brightens");
  private final CompoundParameter core = new CompoundParameter("Core", 30, 0, 100)
    .setDescription("Brightness of the central body");
  private final CompoundParameter coreSize = new CompoundParameter("CoreSz", 30, 0, 100)
    .setDescription("Size of the central body, independent of its brightness");
  private final CompoundParameter shadow = new CompoundParameter("Shadow", 0, 0, 100)
    .setDescription("The planet's shadow falling across the rings");

  // Matter

  private final CompoundParameter trail = new CompoundParameter("Trail", 62, 0, 100)
    .setDescription("Persistence of the orbital streaks");
  private final CompoundParameter grain = new CompoundParameter("Grain", 40, 0, 100)
    .setDescription("Particulate hardness");
  private final CompoundParameter dot = new CompoundParameter("Dot", 45, 0, 100)
    .setDescription("Size and roundness of each orbiting body");
  private final CompoundParameter gain = new CompoundParameter("Gain", 50, 5, 100)
    .setDescription("Density to brightness response");

  // Color

  private final CompoundParameter hue = new CompoundParameter("Hue", 32, 0, 360)
    .setDescription("Hue at the outer edge");
  private final CompoundParameter hueRange = new CompoundParameter("HueRng", -35, -180, 180)
    .setDescription("Hue shift from outer edge inward");
  private final CompoundParameter satDisk = new CompoundParameter("Sat", 55, 0, 100)
    .setDescription("Saturation at the outer edge");
  private final CompoundParameter satRange = new CompoundParameter("SatRng", -40, -100, 100)
    .setDescription("Saturation shift inward");
  private final CompoundParameter hueCore = new CompoundParameter("HueCore", 35, 0, 360)
    .setDescription("Hue of the planet, independent of the rings");
  private final CompoundParameter satCore = new CompoundParameter("SatCore", 25, 0, 100)
    .setDescription("Saturation of the planet, independent of the rings");
  private final CompoundParameter hueBand = new CompoundParameter("HueBand", 0, 0, 180)
    .setDescription("Hue stratification - concentric bands of differing composition");
  private final CompoundParameter hueBandN = new CompoundParameter("BandN", 3, 1, 16)
    .setDescription("How many colour strata across the ring");
  private final CompoundParameter bright = new CompoundParameter("Bright", 88, 0, 100)
    .setDescription("Overall brightness");
  private final CompoundParameter black = new CompoundParameter("Black", 3, 0, 40)
    .setDescription("Cutoff below which pixels go dark");

  // Layout

  private final BooleanParameter wrap = new BooleanParameter("Wrap", true)
    .setDescription("One planet spanning the whole structure instead of one per face");
  private final CompoundParameter cubeFit = new CompoundParameter("CubeFit", 30, 5, 100)
    .setDescription("Framing - how much of the structure the image spans");
  private final CompoundParameter cubeCut = new CompoundParameter("CubeCut", 0, 0, 100)
    .setDescription("Hide the inner region on the cube, so only outer rings appear there");
  private final CompoundParameter posX = new CompoundParameter("PosX", 0, -100, 100)
    .setDescription("Scroll the whole image around the structure");
  private final CompoundParameter posY = new CompoundParameter("PosY", 0, -100, 100)
    .setDescription("Raise or lower the whole image");
  private final CompoundParameter roll = new CompoundParameter("Roll", 0, 0, 360)
    .setDescription("Tip the ring plane - 0 lays it flat around the horizon");
  private final CompoundParameter rollSpd = new CompoundParameter("RollSpd", 0, -100, 100)
    .setDescription("Rotate the whole ring system clockwise");
  private final CompoundParameter limb = new CompoundParameter("Limb", 60, 0, 100)
    .setDescription("Sunlight falling across the planet from one side");
  private final CompoundParameter limbAng = new CompoundParameter("LimbAng", 210, 0, 360)
    .setDescription("Direction the light comes from");

  // Particles - each keeps its orbital elements, not a position

  // Normalised radius; the real orbit is derived from Inner and Outer every
  // frame. Storing an absolute radius meant those knobs only ever reached
  // bodies that did not exist yet.
  private final float[] pN = new float[MAX_P];
  private final float[] pFall = new float[MAX_P];
  private final float[] pPh = new float[MAX_P];   // phase
  private final float[] pZ = new float[MAX_P];    // height above the plane
  private final float[] pW = new float[MAX_P];    // argument of periapsis
  private int liveCount = 0;

  private final float[] fDens = new float[FW * FH];
  private final float[] fRad = new float[FW * FH];
  private final float[] fCore = new float[FW * FH];   // planet, kept off the ring ramp

  private final Random rng = new Random(11235L);
  private float time = 0f;

  private float sGain, sBr, sBk, sGrain, sHue, sHueRng, sSat, sSatRng;
  private float sHueBand, sHueBandN;
  private float sLimb, sLimbX, sLimbY;
  private float sHueCore, sSatCore, sCubeFit, sCubeCut;
  private float sPosX, sPosY;
  private float rollNow = 0f;
  private boolean renderingCube = false;
  private boolean sWrap;
  private float sAspect = 1f;

  public PlanetaryRing(LX lx) {
    super(lx);
    addParameter("Reset",  this.reseed);
    addParameter("Bodies", this.bodies);
    addParameter("Inner",  this.inner);
    addParameter("Outer",  this.outer);
    addParameter("Gaps",   this.gaps);
    addParameter("GapN",   this.gapCount);
    addParameter("GapEdge", this.gapSharp);
    addParameter("Striate", this.striate);
    addParameter("StriN",  this.striateN);
    addParameter("Thick",  this.thickness);
    addParameter("StretchX", this.stretchX);
    addParameter("StretchY", this.stretchY);
    addParameter("Tilt",   this.tilt);
    addParameter("Rate",   this.rate);
    addParameter("Kepler", this.kepler);
    addParameter("Ecc",    this.eccent);
    addParameter("Prec",   this.precess);
    addParameter("Infall", this.infall);
    addParameter("Heat",   this.heat);
    addParameter("Core",   this.core);
    addParameter("CoreSz", this.coreSize);
    addParameter("Shadow", this.shadow);
    addParameter("Trail",  this.trail);
    addParameter("Grain",  this.grain);
    addParameter("Dot",    this.dot);
    addParameter("Gain",   this.gain);
    addParameter("Hue",    this.hue);
    addParameter("HueRng", this.hueRange);
    addParameter("Sat",    this.satDisk);
    addParameter("SatRng", this.satRange);
    addParameter("HueCore", this.hueCore);
    addParameter("SatCore", this.satCore);
    addParameter("HueBand", this.hueBand);
    addParameter("BandN",  this.hueBandN);
    addParameter("Bright", this.bright);
    addParameter("Black",  this.black);
    addParameter("Wrap",   this.wrap);
    addParameter("CubeFit", this.cubeFit);
    addParameter("CubeCut", this.cubeCut);
    addParameter("PosX",   this.posX);
    addParameter("PosY",   this.posY);
    addParameter("Roll",   this.roll);
    addParameter("RollSpd", this.rollSpd);
    addParameter("Limb",   this.limb);
    addParameter("LimbAng", this.limbAng);
    resetBodies();
  }

  // Fixed seed, so Reset always gives exactly the same arrangement.
  private void resetBodies() {
    Random r = new Random(20260809L);
    for (int i = 0; i < MAX_P; i++) {
      pN[i] = r.nextFloat();
      pPh[i] = r.nextFloat() * TWO_PI;
      pZ[i] = r.nextFloat() - 0.5f;
      pW[i] = r.nextFloat() * TWO_PI;
      pFall[i] = 0f;
    }
  }

  @Override
  protected void render(double deltaMs) {
    float dt = (float) Math.min(deltaMs / 1000.0, 0.05);
    time += dt;

    float lo = inner.getValuef() / 100f * 0.5f;
    float hi = outer.getValuef() / 100f * 0.5f;
    if (hi < lo + 0.01f) hi = lo + 0.01f;

    int target = (int) (bodies.getValuef() / 100f * MAX_P);
    if (target < 1) target = 1;
    liveCount = target;

    float spin   = rate.getValuef() / 100f * 1.7f;
    float kep    = kepler.getValuef() / 100f;
    float ecc    = eccent.getValuef() / 100f * 0.85f;
    float prec   = precess.getValuef() / 100f * 0.5f;
    float thick  = thickness.getValuef() / 100f * 0.30f;
    float strX   = stretchX.getValuef() / 100f;
    float strY   = stretchY.getValuef() / 100f;
    float tiltA  = tilt.getValuef() / 100f;
    float gapA   = gaps.getValuef() / 100f;
    float gapEdge = 0.35f + gapSharp.getValuef() / 100f * 2.6f;
    float striA  = striate.getValuef() / 100f;
    float striN  = striateN.getValuef();
    float shadA  = shadow.getValuef() / 100f;
    float dotS   = 0.30f + dot.getValuef() / 100f * 1.05f;
    float dotInv = 1f / (2f * dotS * dotS);
    float dotR2  = 6.25f;
    int   gapN   = clampi(Math.round(gapCount.getValuef()), 1, 8);
    float fall   = infall.getValuef() / 100f;
    float heatA  = heat.getValuef() / 100f;
    float coreA  = core.getValuef() / 100f;
    float decay  = 1f - (1f - trail.getValuef() / 100f) * 0.9f;
    if (decay > 0.965f) decay = 0.965f;

    sGrain  = grain.getValuef() / 100f;
    sGain   = gain.getValuef() / 100f * 8f;
    sBr     = bright.getValuef();
    sBk     = black.getValuef() / 100f;
    sHue    = hue.getValuef();
    sHueRng = hueRange.getValuef();
    sSat    = satDisk.getValuef();
    sSatRng = satRange.getValuef();
    sHueBand = hueBand.getValuef();
    sHueBandN = hueBandN.getValuef();
    sHueCore = hueCore.getValuef();
    sSatCore = satCore.getValuef();
    sWrap  = wrap.isOn();
    sCubeFit = cubeFit.getValuef() / 100f;
    sCubeCut = cubeCut.getValuef() / 100f;
    sPosX = posX.getValuef() / 100f;
    sPosY = posY.getValuef() / 100f * 0.5f;
    rollNow += rollSpd.getValuef() / 100f * dt * 0.9f;
    if (rollNow > 6.2831855f) rollNow -= 6.2831855f;
    else if (rollNow < -6.2831855f) rollNow += 6.2831855f;
    float rollRad = (roll.getValuef() * 3.14159f / 180f) + rollNow;
    float cosRoll = (float) Math.cos(rollRad);
    float sinRoll = (float) Math.sin(rollRad);
    sLimb  = limb.getValuef() / 100f;
    float la = limbAng.getValuef() * 3.14159f / 180f;
    sLimbX = (float) Math.cos(la);
    sLimbY = (float) Math.sin(la);

    for (int i = 0; i < fDens.length; i++) {
      fDens[i] *= decay;
      fRad[i] *= decay;
      fCore[i] = 0f;
    }

    for (int i = 0; i < liveCount; i++) {
      float aN = pN[i] - pFall[i];
      aN = aN - (float) Math.floor(aN);
      float a = lo + aN * Math.max(1e-4f, hi - lo);

      // Kepler's third law: inner orbits run faster. At Kepler=0 the disk turns
      // as a rigid body, which is what a solid ring looks like; at 100 it
      // shears, which is what gravity actually does.
      float rNorm = (a - lo) / Math.max(1e-4f, hi - lo);
      float w = spin * (1f - kep + kep * (float) Math.pow(Math.max(a, 0.02f) / hi, -1.5f) * 0.35f);
      // Wrapped so the angle never grows large enough to lose float
      // precision on a long unbroken run. cos and sin are unchanged by a
      // full turn, so this cannot alter a single pixel.
      pPh[i] += w * dt;
      if (pPh[i] > 6.2831855f) pPh[i] -= 6.2831855f;
      else if (pPh[i] < 0f) pPh[i] += 6.2831855f;
      pW[i] += prec * dt;
      if (pW[i] > 6.2831855f) pW[i] -= 6.2831855f;
      else if (pW[i] < 0f) pW[i] += 6.2831855f;

      if (fall > 0f) {
        pFall[i] += fall * dt * 0.08f;
        if (pFall[i] > 1f) pFall[i] -= 1f;
      } else if (pFall[i] != 0f) {
        pFall[i] -= pFall[i] * Math.min(1f, dt * 0.8f);
        if (Math.abs(pFall[i]) < 1e-4f) pFall[i] = 0f;
      }

      // resonance gaps - matter is cleared at particular radii
      float keep = 1f;
      if (gapA > 0f) {
        // Divisions are narrow swept-clean lanes at specific radii, not a
        // smooth ripple - the Cassini division is genuinely empty.
        float nearest = Float.MAX_VALUE;
        for (int k = 1; k <= gapN; k++) {
          float gr = (float) k / (gapN + 1);
          float dd = Math.abs(rNorm - gr);
          if (dd < nearest) nearest = dd;
        }
        float gw = 0.055f / gapEdge;
        float notch = (float) Math.exp(-(nearest * nearest) / (2f * gw * gw));
        keep = 1f - gapA * notch;
        if (keep < 0f) keep = 0f;
      }
      // Saturn's rings are not a few broad bands but thousands of ringlets;
      // a fine density ripple is what gives them their grooved look.
      if (striA > 0f) {
        float r2 = 0.5f + 0.5f * (float) Math.cos(rNorm * striN * 6.2832f);
        keep *= 1f - striA * 0.75f * (1f - r2);
      }
      if (keep < 0.02f) continue;

      float ph = pPh[i];
      float r = a * (1f - ecc * ecc) / (1f + ecc * (float) Math.cos(ph - pW[i]));

      // Tilt squashes the circle into the ellipse we see from an angle; Roll
      // then turns that ellipse in the image plane, so the ring can lie flat
      // around the horizon instead of running top to bottom.
      // Stretch measured from the inner edge outward, so the inner
      // circumference stays put while the ring sweeps further across the wall.
      float rx = lo + (r - lo) * strX;
      float ry = lo + (r - lo) * strY;
      float ringX = (float) Math.cos(ph) * rx;
      float ringY = (float) Math.sin(ph) * ry * (1f - tiltA * 0.92f)
                  + pZ[i] * thick * (1f - tiltA * 0.5f);
      float x = 0.5f + ringX * cosRoll - ringY * sinRoll;
      float y = 0.5f + ringX * sinRoll + ringY * cosRoll;

      // the planet casts a shadow across the far side of the ring
      if (shadA > 0f) {
        float sx = (float) Math.cos(ph - rollRad);
        if (sx < -0.15f && Math.abs((float) Math.sin(ph)) < 0.55f) {
          keep *= 1f - shadA * Math.min(1f, (-sx - 0.15f) * 2.2f);
          if (keep < 0.02f) continue;
        }
      }

      if (x < 0f || x > 1f || y < 0f || y > 1f) continue;

      float gx = x * (FW - 1), gy = y * (FH - 1);
      int gx0 = (int) gx, gy0 = (int) gy;
      if (gx0 < 1 || gy0 < 1 || gx0 >= FW - 2 || gy0 >= FH - 2) continue;
      // inner orbits are hotter, which is the accretion disk signature
      float wgt = keep * (1f + heatA * 3.5f * (1f - rNorm));
      // A round falloff rather than a 2x2 bilinear box. The box is what made
      // each body render as a little square once upsampled to the wall.
      for (int oy = -1; oy <= 1; oy++) {
        int cy = gy0 + oy;
        float ddy = (cy + 0.5f) - gy;
        for (int ox = -1; ox <= 1; ox++) {
          int cx = gx0 + ox;
          float ddx = (cx + 0.5f) - gx;
          float r2 = ddx * ddx + ddy * ddy;
          if (r2 > dotR2) continue;
          float dw = (float) Math.exp(-r2 * dotInv) * wgt;
          int c = cy * FW + cx;
          fDens[c] += dw;
          fRad[c] += dw * rNorm;
        }
      }
    }

    // central body
    if (coreA > 0f) {
      float cr = 0.010f + coreSize.getValuef() / 100f * 0.13f;
      int x0 = (int) ((0.5f - cr) * FW), x1 = (int) ((0.5f + cr) * FW) + 1;
      int y0 = (int) ((0.5f - cr) * FH), y1 = (int) ((0.5f + cr) * FH) + 1;
      if (x0 < 0) x0 = 0; if (y0 < 0) y0 = 0;
      if (x1 > FW) x1 = FW; if (y1 > FH) y1 = FH;
      float lim = limb.getValuef() / 100f;
      float lax = (float) Math.cos(limbAng.getValuef() * 3.14159f / 180f);
      float lay = (float) Math.sin(limbAng.getValuef() * 3.14159f / 180f);
      for (int gy = y0; gy < y1; gy++) {
        float dy = (gy + 0.5f) / FH - 0.5f;
        for (int gx = x0; gx < x1; gx++) {
          float dx = (gx + 0.5f) / FW - 0.5f;
          float d = (float) Math.sqrt(dx * dx + dy * dy);
          if (d > cr) continue;
          // A lit sphere, not a flat disc: brightness follows the angle
          // between the surface normal and the light. This is most of what
          // makes it read as a body rather than a circle.
          float shade = 1f;
          if (lim > 0f) {
            float nx = dx / cr, ny = dy / cr;
            float nzc = (float) Math.sqrt(Math.max(0f, 1f - nx * nx - ny * ny));
            float lambert = clamp01(nx * lax + ny * lay + nzc * 0.55f);
            shade = (1f - lim) + lim * lambert;
          }
          float edge = clamp01((cr - d) / (cr * 0.18f));
          int c = gy * FW + gx;
          float add = shade * edge * coreA * 20f;
          if (add > fDens[c]) fDens[c] = add;
          float k = shade * edge;
          if (k > fCore[c]) fCore[c] = k;
        }
      }
    }

    renderCube();
    renderCylinder();
    copyExterior();
  }

  private void renderCube() {
    Cube cube = Apotheneum.cube;
    if (cube == null) return;
    renderingCube = true;
    Face[] ext = cube.exterior.faces;
    for (int f = 0; f < ext.length; f++) {
      renderCubeFace(ext[f], false, f, ext.length);
    }
    renderingCube = false;
  }

  private void renderCubeFace(Face face, boolean flip, int faceIdx, int faceCount) {
    int cols = Apotheneum.GRID_WIDTH, rows = face.rows.length;
    // Wrapped, the four faces are one continuous canvas 200 pixels around.
    // Aspect correction keeps the planet a circle rather than an ellipse.
    sAspect = sWrap ? (float) (cols * faceCount) / rows : 1f;
    for (int ri = 0; ri < rows; ri++) {
      Row row = face.rows[ri];
      float v = (float) ri / (rows - 1);
      for (int ci = 0; ci < cols; ci++) {
        float u;
        if (sWrap) {
          u = (float) (faceIdx * cols + ci) / (cols * faceCount - 1);
        } else {
          u = (float) ci / (cols - 1);
          if (flip) u = 1f - u;
        }
        colors[row.points[ci].index] = sampleField(u, v);
      }
    }
  }

  private void renderCylinder() {
    Cylinder cyl = Apotheneum.cylinder;
    if (cyl == null) return;
    Ring[] rings = cyl.exterior.rings;
    int numRings = rings.length;
    renderingCube = false;
    if (sWrap) sAspect = (float) rings[0].points.length / numRings;
    for (int ri = 0; ri < numRings; ri++) {
      Ring ring = rings[ri];
      int np = ring.points.length;
      float v = (float) ri / (numRings - 1);
      for (int pi = 0; pi < np; pi++) {
        float u = (float) pi / (np - 1);
        colors[ring.points[pi].index] = sampleField(u, v);
      }
    }
  }

  private int sampleField(float u, float v) {
    if (sWrap) {
      // scrolling happens in wall space, so the image travels around the
      // structure and across the corners continuously
      u -= sPosX;
      u = u - (float) Math.floor(u);
      v -= sPosY;
      // one field unit becomes the same number of pixels horizontally and
      // vertically, so circles stay circular however wide the canvas is.
      // The cube can be framed wider than the cylinder, so the ring system
      // reads as continuing outward past the inner volume.
      float framing = sCubeFit;
      float z = 1f / Math.max(0.05f, framing);
      u = 0.5f + (u - 0.5f) * z;
      v = 0.5f + (v - 0.5f) * z / Math.max(0.05f, sAspect);
      if (u < 0f || u > 1f || v < 0f || v > 1f) return LXColor.BLACK;
    }
    float gx = u * (FW - 1), gy = v * (FH - 1);
    int x0 = (int) gx, y0 = (int) gy;
    int x1 = x0 + 1, y1 = y0 + 1;
    if (x1 >= FW) x1 = FW - 1;
    if (y1 >= FH) y1 = FH - 1;
    float tx = gx - x0, ty = gy - y0;
    int c00 = y0 * FW + x0, c10 = y0 * FW + x1;
    int c01 = y1 * FW + x0, c11 = y1 * FW + x1;

    float d = bilerp(fDens[c00], fDens[c10], fDens[c01], fDens[c11], tx, ty);
    if (d < 1e-4f) return LXColor.BLACK;
    float rad = bilerp(fRad[c00], fRad[c10], fRad[c01], fRad[c11], tx, ty) / d;
    float coreness = bilerp(fCore[c00], fCore[c10], fCore[c01], fCore[c11], tx, ty);

    // on the cube, optionally show only the outer rings
    if (renderingCube && sCubeCut > 0f) {
      float rr = (float) Math.sqrt((u - 0.5f) * (u - 0.5f) + (v - 0.5f) * (v - 0.5f));
      float cut = sCubeCut * 0.5f;
      if (rr < cut) return LXColor.BLACK;
      float feather = cut * 0.25f;
      if (rr < cut + feather) d *= (rr - cut) / feather;
    }

    float bri = 1f - (float) Math.exp(-d * sGain);
    if (sGrain > 0f) {
      float hard = bri * bri * (3f - 2f * bri);
      bri = bri + (hard - bri) * sGrain;
    }
    bri *= sBr / 100f;
    if (bri < sBk) return LXColor.BLACK;

    float inward = clamp01(1f - rad);
    float h = sHue + sHueRng * inward;
    // Concentric colour strata: the rings differ in composition band to band,
    // which is what the false-colour Cassini images make visible.
    if (sHueBand > 0f) {
      h += sHueBand * (float) Math.sin(rad * sHueBandN * 6.2832f);
    }
    float s = sSat + sSatRng * inward;

    // The planet has its own hue and saturation. Previously it shared the
    // ring's radial ramp, which is why the two could never be set apart.
    float k = clamp01(coreness * 1.6f);
    if (k > 0.01f) {
      h = h + (sHueCore - h) * k;
      s = s + (sSatCore - s) * k;
    }
    h = ((h % 360f) + 360f) % 360f;

    return LXColor.hsb(h, clamp01(s / 100f) * 100f, clamp01(bri) * 100f);
  }

  private static float bilerp(float c00, float c10, float c01, float c11, float tx, float ty) {
    float a = c00 + (c10 - c00) * tx;
    float b = c01 + (c11 - c01) * tx;
    return a + (b - a) * ty;
  }

  private static float clamp01(float v) { return v < 0f ? 0f : v > 1f ? 1f : v; }

  private static int clampi(int v, int lo, int hi) { return v < lo ? lo : v > hi ? hi : v; }
}
