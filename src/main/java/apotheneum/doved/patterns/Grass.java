...
public class Grass extends ColorNativePattern {
  ...
  private static final double DEFAULT_TIP_TONE_MIX = .8;
  ...
  public final CompoundParameter silvering =
    new CompoundParameter("Silver", .25, 0, 1)
    .setDescription("How strongly wind exposes the paler secondary blade tone");

  public final CompoundParameter tipTone =
    new CompoundParameter("Tone", DEFAULT_TIP_TONE_MIX, 0, 1)
    .setDescription("How completely blade tips take the secondary tone");
  ...
  private class Field {
    ...
    private final float[] primaryBuffer;
    private final float[] secondaryBuffer;
    ...
    private Field(int width, int height, int segmentWidth) {
      ...
      this.primaryBuffer = new float[width * height];
      this.secondaryBuffer = new float[width * height];
    ...
    private void run(double dt, Apotheneum.Orientation orientation) {
      ...
      Arrays.fill(this.primaryBuffer, 0f);
      Arrays.fill(this.secondaryBuffer, 0f);
      ...
      for (int i = 0; i < active; ++i) {
        ...
        final double outOfPlaneBend = LXUtils.clamp(
          MAX_OUT_OF_PLANE_BEND * strength * windNormal * blade.dragScale / springStiffness,
    ...
    private void draw(Blade blade, double outOfPlaneBend) {
      ...
      final double windTone = LXUtils.clamp(
        (Math.abs(bend) / MAX_BEND + outOfPlaneBend / MAX_OUT_OF_PLANE_BEND) * .5,
        0, 1);
      ...
      for (int i = 1; i <= steps; ++i) {
        ...
        final double secondaryMix = LXUtils.clamp(
          tipToneValue * s + silveringValue * windTone, 0, 1);
        splat(x, y, deposit * level(s), secondaryMix, segmentStart);
      }
    }

    private void splat(double x, double y, double amount, double secondaryMix, int segmentStart) {
      ...
      if (this.segmentWidth == 0) {
        ...
        add(wrappedX0, y0, amount * wx0 * wy0, secondaryMix);
        add(wrappedX1, y0, amount * wx1 * wy0, secondaryMix);
        add(wrappedX0, y0 + 1, amount * wx0 * wy1, secondaryMix);
        add(wrappedX1, y0 + 1, amount * wx1 * wy1, secondaryMix);
        return;
      }
      final int segmentEnd = segmentStart + this.segmentWidth;
      if ((x0 >= segmentStart) && (x0 < segmentEnd)) {
        add(x0, y0, amount * wx0 * wy0, secondaryMix);
        add(x0, y0 + 1, amount * wx0 * wy1, secondaryMix);
      }
      if ((x1 >= segmentStart) && (x1 < segmentEnd)) {
        add(x1, y0, amount * wx1 * wy0, secondaryMix);
        add(x1, y0 + 1, amount * wx1 * wy1, secondaryMix);
      }
    }

    private void add(int x, int y, double amount, double secondaryMix) {
      if ((y < 0) || (y >= this.height)) {
        return;
      }
      final int index = y * this.width + x;
      this.buffer[index] += (float) amount;
      this.primaryBuffer[index] += (float) (amount * (1 - secondaryMix));
      this.secondaryBuffer[index] += (float) (amount * secondaryMix);
    }
      ...
      for (int c = 0; c < columns.length; ++c) {
        ...
        for (int y = 0; y < available; ++y) {
          ...
          if (value <= 0) {
            ...
            colors[points[y].index] = LXColor.gray(100 * Math.min(1, value));
          } else {
  ...
  private double silveringValue;
  private double tipToneValue;
  ...
  public Grass(LX lx) {
    ...
    addParameter("silvering", this.silvering);
    addParameter("tipTone", this.tipTone);
  }
  ...
  protected void render(double deltaMs) {
    ...
    this.silveringValue = this.silvering.getValue();
    this.tipToneValue = this.tipTone.getValue();
    this.primary.update();
  ...
  public void buildDeviceControls(UI ui, UIDevice uiDevice, ColorNativePattern pattern) {
    ...
    addColumn(uiDevice, "Field",
      ...
      newKnob(this.bladeHeight),
      newKnob(this.heightVariation)
    ).setChildSpacing(6);

    addVerticalBreak(ui, uiDevice);

    addColumn(uiDevice, "Blade",
      newKnob(this.stiffness),
      newKnob(this.damping),
      newKnob(this.tipGlow)
    ).setChildSpacing(6);

    addVerticalBreak(ui, uiDevice);

    addColumn(uiDevice, "Render",
      newKnob(this.sharpness),
      newButton(this.reseed).setTriggerable(true)
    ).setChildSpacing(6);

    addVerticalBreak(ui, uiDevice);

    addColumn(uiDevice, "Tone",
      newKnob(this.silvering),
      newKnob(this.tipTone)
    ).setChildSpacing(6);
