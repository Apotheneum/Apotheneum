...
/**
 * Performance-optimized version of {@link Bursts}. Behavior and parameters
 * are identical - only the per-frame work each Burst does has changed:
 *
 *  - Burst.render() previously recomputed the column-only term (pxn/dx, which
 *    involves a division and a wrapdiff() call) once per POINT, even though it
 *    only varies by column - so it was redundantly recomputed once per row
 *    within that column. Same story for the row-only term (pyn/dy). Both are
 *    now precomputed once per column / once per row into small reusable
 *    arrays, so the inner per-point loop only does the work that actually
 *    depends on both x and y (the spin matrix multiply and distance/shape
 *    evaluation).
 *  - The outer render() loop used to do a full render pass, then a second
...
public abstract class BurstsOptimized extends ApotheneumPattern implements ApotheneumPattern.Midi {
  ...
  protected class Burst {
    ...
    // Precomputed once per burst (not once per point!) - dx only depends on
    // column index, dy only depends on row index, so each is computed once
    // per render() call into these reusable arrays instead of once per pixel.
    private final float[] dxCache;
    private final float[] dyCache;
    ...
    protected Burst(Apotheneum.Column[] columns, Burst copy) {
      ...
      this.dxCache = new float[columns.length];
      this.dyCache = new float[columns[0].points.length];
    ...
    protected void render(double deltaMs) {
      ...
      if (this.basis < 1) {
        ...
        final int numRows = this.dyCache.length;

        for (int x = 0; x < numCols; x++) {
          float pxn = x / (numCols - wrapOffset);
          this.dxCache[x] = wrap ? 3f * wrapdiff(this.xn, pxn) : this.xn - pxn;
        }
        for (int y = 0; y < numRows; y++) {
          float pyn = y / (numRows - 1f);
          this.dyCache[y] = this.yn - pyn;
        }

        int x = 0;
        for (Apotheneum.Column column : this.columns) {
          final float dx = this.dxCache[x];
          int y = 0;
          for (LXPoint p : column.points) {
            final float dy = this.dyCache[y];
